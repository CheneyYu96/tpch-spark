...
class Q09 extends TpchQuery {
  ...
  override def getRawSQL(): String = {
    return "select nation, o_year, sum(amount) as sum_profit \nfrom (select n_name as nation, o_orderdate as o_year, l_extendedprice * (1 - l_discount) - ps_supplycost * l_quantity as amount from PART, SUPPLIER, LINEITEM, PARTSUPP, ORDERS, NATION where s_suppkey = l_suppkey and ps_suppkey = l_suppkey and ps_partkey = l_partkey and p_partkey = l_partkey and o_orderkey = l_orderkey and s_nationkey = n_nationkey and p_name like '%dim%') profit \ngroup by nation, o_year \norder by nation, o_year desc"
  }
